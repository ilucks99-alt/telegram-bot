...
HELP_TEXT = """
한화생명 대체투자 포트폴리오 Bot
(Dataset 기준: 26.7월 / LookThrough 기준: 26.6월)
...
📋 [업무 지시]
/등록 이름 — 팀원 등록
/지시 이름 | 업무내용 [| project=BS00001505] [| due=15:00 또는 due=2026-04-29 15:00]
  · 담당자에게 [✅ 확인했습니다] 버튼이 함께 전송되며, 누르면 본인에게 알림이 옵니다.
  · due 지정 시 마감 30분 전에 담당자/본인에게 자동 푸시됩니다.
...
만기년도·만기월·만기일 / 최초인출일 / IRR / 약정 / 콜 / 상환 / 잔액 / NAV / DPI / TVPI /
...
def process_user_message(db: InvestmentDB, chat_id: int, text: str, ctx: Dict[str, Any]) -> None:
    ...
    if raw.startswith("/대체투자뉴스"):
        if str(chat_id) != str(config.OWNER_CHAT_ID):
            ...
            return
        handle_alternative_news_command(db, chat_id)
        return
